...
private:
    ...
    void SendMove(tcp::socket &socket, std::pair<int, int> move){
        auto move_str = MoveToString(move);

        socket.write_some(net::buffer(move_str));
    }

    void ReadResult(tcp::socket &socket)
    {
        net::streambuf streambuf;

        socket.read_some(streambuf);

        SeabattleField::ShotResult result = streambuf.[0];
